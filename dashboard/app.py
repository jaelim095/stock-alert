...
with tab_pf:
    ...
    with st.expander("이 표 읽는 법 — 이동평균선 기초와 교과서적 해석"):
        st.markdown(f"""
##### 각 열의 의미

- **vs N일%**: 현재 종가가 N일 이동평균선(최근 N거래일 종가의 평균)보다 몇 % 위(+, 빨강)
  또는 아래(-, 파랑)에 있는지. 0에 가까울수록 그 선 바로 위에 서 있는 상태

##### 각 선의 통상적 별칭과 용도

| 선 | 별칭 | 보는 법 |
|---|---|---|
| 5일 | 단기 심리선 | 최근 1주 흐름. 단타 영역이라 장기 투자에선 참고만 |
| 20일 | 생명선 | 약 한 달 추세. 상승 종목이 눌릴 때 지지선으로 가장 많이 보는 선 |
| 60일 | 수급선 | 분기 추세. 기관 자금의 방향으로 해석 |
| 120일 | 경기선 | 반년 추세. 장기 상승/하락의 기준선 |

##### 교과서적 해석 (일반론)

- **매수 쪽에서 보는 자리**: ① 정배열(5>20>60>120, 상승 추세) 종목이 20일선 근처까지
  눌렸다가 지지될 때 — 표에서 배열=정배열이면서 vs 20일%가 0 근처
  ② 골든크로스(20일선이 60일선을 위로 돌파) 발생 직후
- **매도·주의 쪽에서 보는 자리**: ① 종가가 20일선을 명확히 하향 이탈(vs 20일%가 마이너스 전환)
  ② 데드크로스 ③ 역배열 전환 — 하락 추세 확정으로 해석
- **이격 과다**: vs 20일%가 ±15%를 넘으면 단기 과열/과매도 구간 —
  추격 매수나 투매 모두 주의
- **역배열 종목의 반등**: 단명하는 경우가 많아 교과서적으로는 신규 매수 자제가 기본

##### 주의 (반드시 읽기)

- 이동평균은 **후행 지표**입니다 — 신호가 늦고, 횡보장에서는 가짜 크로스가 잦습니다
- 레버리지 2x ETF({', '.join(config.UNDERLYING_MAP)})는 감쇠 때문에 이 문법이 왜곡됩니다 —
  추세 판단은 기초자산({', '.join(config.UNDERLYING_MAP.values())}) 기준으로 하세요
- 이 표는 참고 정보이며, 매매 판단과 실행은 항상 본인 몫입니다 (이 시스템의 원칙)
""")
